...
parser.add_argument("--baidex", "-ba", help = "path to input related bai index file for pysam")
...
if(args.feather is not None):
    ...
    with open(tempfn, 'rb') as tempfh:
        ...
        #cashel edit here
        reads_all = pd.concat([reads_all,pd.DataFrame(np.empty(sum(c[1:])*ncols).reshape(-1,ncols), columns = colnames)])
        #reads_all = reads_all.concat(pd.DataFrame(np.empty(sum(c[1:])*4).reshape(-1,4)))
